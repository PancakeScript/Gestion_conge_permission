...
const getDashboardManager = async (id_manager) => {
  ...
  if (demandesRetard.length > 0) {
    ...
    if (managerUser?.id_utilisateur) {
      const aNotifier = demandesRetard.filter(d => !d.rappel_envoye);
      if (aNotifier.length > 0) {
        await prisma.notification.createMany({
          data: aNotifier.map(d => ({
        ...
        await prisma.demandes_conge.updateMany({
          where: { id_demande_conde: { in: aNotifier.map(d => d.id_demande_conde) } },
          data: { rappel_envoye: true },
        });
      }
    }
  }
