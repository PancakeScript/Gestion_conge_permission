...
export default function LoginManager() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Vérifier que le rôle est bien "manager"
      if (data.role !== "manager") {
        throw new Error("Ce compte n'est pas un compte manager.");
      }
      login(data.token, {
        role: data.role,
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .input-icon { position: absolute; left: 15px; top: 50%; transform: translateY(-50%); color: #b8a892; pointer-events: none; }
        ...
        .toggle-pw { position: absolute; right: 14px; top: 50%; transform: translateY(-50%); background: none; border: none; cursor: pointer; color: #b8a892; }
        ...
        .error-box { background: #fef5f5; border: 1px solid #f5c0c0; border-radius: 8px; padding: 11px 14px; font-size: 13px; color: #c0392b; margin-bottom: 20px; display: flex; align-items: center; gap: 8px; }
      ...
      <div className="login-root">
        <div className="left-panel">
          ...
          <div className="left-content">
            <h1 className="left-title">Gestion<br /><em>d'équipe</em></h1>
            <div className="divider"></div>
            <p className="left-desc">Espace manager : validez les congés, visualisez le planning de votre département.</p>
          </div>
          <div className="stats-row">
            <div className="stat"><span className="stat-num">Suivi</span><span className="stat-label">Temps réel</span></div>
            <div className="stat"><span className="stat-num">Équipe</span><span className="stat-label">Centralisé</span></div>
            <div className="stat"><span className="stat-num">Validations</span><span className="stat-label">Simplifiées</span></div>
          </div>
        </div>

        <div className="right-panel">
          <p className="login-label">Connexion Manager</p>
          <h2 className="login-title">Bon retour</h2>
          <p className="login-subtitle">Gérez les absences de votre équipe</p>
          ...
          <form onSubmit={handleSubmit}>
            <div className="form-group">
              ...
              <div className="input-wrap">
                ...
                <input type="email" className="form-input" placeholder="manager@exemple.com"
            ...
            <button type="submit" className="btn-login" disabled={loading}>
              {loading ? <><div className="spinner"/> Connexion...</> : <>Se connecter <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg></>}
            </button>
          </form>

          <div className="register-link">
            Pas encore de compte manager ? <a onClick={() => navigate('/register-manager')}>Créer un compte manager</a>
          </div>
        </div>
      </div>
    </>
